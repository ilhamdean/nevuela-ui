...
const basicItems: CommandItem[] = [
  ...
  { id: 'instances', label: 'Go to Instances', group: 'Pages', icon: Server },
  { id: 'billing', label: 'Go to Billing', group: 'Pages', icon: CreditCard },
  { id: 'new-instance', label: 'Create Instance', group: 'Actions', icon: Plus },
  { id: 'invite', label: 'Invite teammate', group: 'Actions', icon: UserPlus },
  { id: 'recent-1', label: 'web-prod-01', group: 'Recent', icon: Server },
  { id: 'recent-2', label: 'db-staging-02', group: 'Recent', icon: Database },
]
...
/**
 * A realistic console command set — page navigation, resource creation, and
 * help, each carrying a keyboard shortcut hint rendered via `KbdShortcut`.
 */
export const ConsoleCommands: Story = {
  render: (args) => ({
    components: { CommandPalette, Button },
    setup() {
      const open = ref(false)
