...
/**
 * `StatusBadge` communicates the state of a resource (an instance, database,
 * deployment…). Color is never the only signal — a text label is always present
  ...
  render: (args) => ({
    ...
    template: `
      <div class="flex flex-col items-start gap-2">
        ...
        <StatusBadge v-bind="args" status="warning" label="Rebooting" />
        <StatusBadge v-bind="args" status="error" label="Failed" />
        <StatusBadge v-bind="args" status="off" label="Powered off" />
...
/** In context: a compact resource list. */
export const InResourceList: Story = {
  render: (args) => ({
    components: { StatusBadge },
    setup: () => ({ args }),
    template: `
      <ul class="w-80 divide-y divide-border overflow-hidden rounded-xl border border-border bg-surface">
        <li class="flex items-center justify-between px-4 py-3">
          <span class="text-sm text-fg">web-prod-01</span>
          <StatusBadge status="active" label="Active" />
        </li>
        <li class="flex items-center justify-between px-4 py-3">
          <span class="text-sm text-fg">db-staging</span>
          <StatusBadge status="warning" label="Rebooting" />
        </li>
        <li class="flex items-center justify-between px-4 py-3">
          <span class="text-sm text-fg">worker-02</span>
          <StatusBadge status="off" label="Powered off" />
        </li>
      </ul>`,
  }),
  args: { label: '' },
