...
import { ref } from 'vue'
import { Box, Sparkles, Tag } from '@lucide/vue'
import { Badge } from '.'

/**
 * `Badge` is a small generic label/tag chip for arbitrary categorical or
 * informational text — plan tiers, categories, record types, feature flags.
...
export const WithIcon: Story = {
  render: (args) => ({
    components: { Badge, Tag, Box, Sparkles },
    setup: () => ({ args, Tag, Box, Sparkles }),
    template: `
      <div class="flex flex-wrap items-center gap-2">
        <Badge v-bind="args" color="neutral" :icon="Tag" label="wholesale" />
        <Badge v-bind="args" color="brand" :icon="Sparkles" label="Pro" />
        <Badge v-bind="args" color="blue" :icon="Box" label="Backordered" />
      </div>`,
  }),
  args: { label: '' },
}
...
export const Removable: Story = {
  render: (args) => ({
    components: { Badge },
    setup: () => {
      const filters = ref(['Unpaid', 'Online store', 'Tagged: wholesale'])
...
/** In context: category/plan-tier badges on a record card. */
export const RecordTags: Story = {
  render: () => ({
    components: { Badge },
    template: `
      <div class="flex w-80 flex-col gap-2 rounded-xl border border-border bg-surface p-4">
        <span class="text-sm font-medium text-fg">Halden &amp; Co.</span>
        <div class="flex flex-wrap items-center gap-1.5">
          <Badge color="brand" label="Pro" />
          <Badge color="blue" label="Wholesale" />
          <Badge color="neutral" label="Net 30" />
