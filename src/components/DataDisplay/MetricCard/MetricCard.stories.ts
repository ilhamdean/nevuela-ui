...
const meta = {
  ...
  render: (args) => ({
    components: { MetricCard, ShoppingCart },
    setup: () => ({ args, ShoppingCart }),
    template: `<div class="w-64"><MetricCard v-bind="args" :icon="ShoppingCart" /></div>`,
  }),
  ...
  render: (args) => ({
    ...
    template: `
      <div class="grid w-[720px] grid-cols-3 gap-4">
        <MetricCard label="Orders" value="1.2" unit="k" :delta="{ value: 8.1, direction: 'up', tone: 'positive' }" help-text="of a 4k monthly target">
          <template #sparkline>
        ...
        <MetricCard label="Sessions" value="94.2k" :delta="{ value: 2.3, direction: 'down', tone: 'neutral' }" help-text="Last hour" />
        <MetricCard label="Refund rate" value="0.42" unit="%" :delta="{ value: 15, direction: 'up', tone: 'negative' }" help-text="of paid orders" />
      </div>`,
  }),
}
