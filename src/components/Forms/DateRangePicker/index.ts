...
/**
 * A start/end date pair, exposed to consumers as plain `Date` objects (or
 * `undefined` when unset) rather than `@internationalized/date`'s `DateValue`
 * — the component converts internally so callers never import that package.
 */
...
export const dateRangePickerTriggerVariants = cva(
  [
    'inline-flex w-full items-center gap-1.5 rounded-sm border bg-surface text-sm text-fg',
...
export type DateRangePickerVariants = VariantProps<typeof dateRangePickerTriggerVariants>
