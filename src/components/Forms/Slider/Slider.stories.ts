 ...
 * filters — e.g. "CPU threshold %" or "price range $" in a list-page filter
...
/** Realistic console usage: a CPU alert threshold filter with a `%` suffix. */
export const CpuAlertThreshold: Story = {
  render: (args) => ({
    components: { Slider },
    setup: () => ({ args }),
    template: `
