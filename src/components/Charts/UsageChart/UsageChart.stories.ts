 ...
 * usage and volume graphs over time. It reads the Nevuela chart-color tokens at runtime, so
...
const meta = {
  ...
  args: {
    categories: hours,
    series: [
      { name: 'Orders', data: wave(38, 18, 0) },
      { name: 'Sessions', data: wave(55, 10, 4) },
    ],
    type: 'line',
    area: true,
    height: 300,
...
export const SingleSeriesLine: Story = {
  args: {
    type: 'line',
    area: false,
    showLegend: false,
    series: [{ name: 'Revenue', data: wave(120, 60, 2) }],
    yFormatter: (v: number) => `$${v}k`,
  },
}
...
export const BarVariant: Story = {
  args: {
    ...
    series: [
      { name: 'Orders', data: [1200, 1900, 1700, 2400, 2100, 900, 700] },
      { name: 'Refunds', data: [12, 30, 18, 42, 25, 6, 4] },
    ],
  },
}
