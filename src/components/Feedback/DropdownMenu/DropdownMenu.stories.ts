...
import { Archive, ArrowUpDown, Copy, FolderInput, Pencil, Trash2 } from '@lucide/vue'
import { ref } from 'vue'
...
const items: DropdownEntry[] = [
  ...
  { type: 'separator' },
  { label: 'Delete', icon: Trash2, danger: true, value: 'delete' },
]
...
const submenuItems: DropdownEntry[] = [
  ...
  { label: 'Duplicate', icon: Copy, shortcut: '⌘D', value: 'duplicate' },
  {
    ...
    children: [
      ...
      { label: 'Ready to ship', value: 'ready-to-ship' },
      {
        ...
        value: 'archived',
        children: [
          { label: 'Archived · 2026', value: 'archived-2026' },
          { label: 'Archived · 2025', value: 'archived-2025' },
          { label: 'Archived · 2024', value: 'archived-2024' },
        ],
      },
  ...
  { type: 'separator' },
  { label: 'Delete order', icon: Trash2, danger: true, value: 'delete-order' },
]

/**
 * An `item` entry with `children` renders as a submenu trigger — here a
 * "Move to" action opens a nested folder picker, one level of which nests a
 * further archive-year picker to show submenus can go arbitrarily deep.
 */
export const WithSubmenu: Story = {
  ...
  render: (args) => ({
    ...
    template: `
      <div class="flex justify-center p-12">
        <DropdownMenu v-bind="args" @select="(item) => console.log('select', item.value)">
          <template #trigger><Button variant="secondary">Order actions</Button></template>
        </DropdownMenu>
      </div>`,
  }),
}
