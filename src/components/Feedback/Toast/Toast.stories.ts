 ...
 * Firing several in a row demonstrates the stack — a common console flow
...
export const QueueStacking: Story = {
  ...
  render: () => ({
    ...
    setup() {
      ...
      const deleted = () =>
        toast.success('3 snapshots deleted', {
          description: 'Removed from nyc1-backups.',
        })
      const failed = () =>
        toast.error('Deployment failed', {
          description: 'Health checks did not pass within 5 minutes.',
          actionLabel: 'View logs',
        })
      const quota = () =>
        toast.warning('Approaching quota', {
          description: "You've used 82% of your transfer allowance.",
        })
      const maintenance = () =>
        toast.info('Maintenance scheduled', {
          description: 'NYC1 network maintenance Saturday 02:00–04:00 UTC.',
        })
      const bulk = () => {
        deleted()
        created()
